...
Select "Simple Payment"
Enter: name/label (e.g., "Guitar Lesson"), amount (e.g., $75.00)
Optionally add: payment note, support email, redirect URL
Click "Create Link"
Share the link
Required Fields:
...
Enter item details: name, description, price, image(s)
Configure variants (optional): size, color, material — each with its own price and stock
Set inventory: total quantity available (or unlimited)
Configure fulfillment: Ship or Pickup
If Ship: set shipping fee (flat rate), toggle address collection
If Pickup: select pickup location, set pickup instructions
Configure taxes (auto-calculated or manual)
...
Enter event details: name, date/time, venue/location, description
Configure ticket tiers: name, price, quantity per tier
Set per-order limit (optional): max tickets per purchase
Add custom fields: attendee name, dietary restrictions, etc.
Set event image/banner
Review and create link
Required Fields:
...
Select "Donation"
Enter cause details: name, description, image
Configure amount options:
Preset tiers (e.g., $10, $25, $50, $100) — OR
Single suggested amount with "pay what you want" — OR
Fully open (buyer enters any amount)
Add custom fields (optional): donor name, dedication message, anonymity toggle
Review and create link
Required Fields:
...
Multiple links — one link per tier (simplest, but seller manages multiple links)
Single link with item variations — each tier is a catalog item variation (buyer selects on checkout)
App-layer landing page — the app shows a tier selection page, then redirects to the appropriate QuickPay link (best UX, requires an intermediate page)
...
Enter: service description, amount
Optionally pre-populate buyer info (if sending to a known client)
Add payment note (invoice #, project name)
Add custom fields (optional): project reference, service date
Review and create link
Required Fields:
...
Select "Subscription"
Choose an existing subscription plan from their catalog — OR create a new one
Configure plan details: name, price, billing cadence, trial period
Set fulfillment type (depends on what's being subscribed to)
Review and create link
Required Fields:
...
Select "Food Order"
Build the menu: items with modifiers (sizes, toppings, add-ons)
Configure fulfillment: Pickup or Delivery
...
Enter product details: name, description, price, preview image
Configure delivery method: download URL, access code, or email delivery
Set inventory (optional): limited edition or unlimited
Review and create link
Required Fields:
...
Select "Invoice"
Enter: description, amount, invoice/reference number
Pre-populate buyer info (name, email)
Review and create link
Required Fields:
...
Select "Multi-Item Cart"
Add items: name, price, quantity, image (for each item)
Configure variants per item (optional)
Set inventory per item (optional)
Configure shared fulfillment: Ship or Pickup
Set shipping fee, taxes, discounts
Review and create link
Required Fields:
...
View order with shipping address
Pack and ship the item
Update fulfillment with carrier and tracking number (via Orders API)
Fulfillment state moves: PROPOSED → RESERVED → PREPARED → COMPLETED
Data Model:
...
Pre-built templates — shipped with the app for common use cases
Custom templates — seller saves a link configuration as a template
From existing link — "Save as template" action on any link
Pre-Built Templates:
